...
def main() -> None:
    ...
    missing_pair_cols = [col for col in REQUIRED_PAIR_COLUMNS if col not in pairs_df.columns]
    ...
    for barrier_json in sorted(BARRIER_ROOT.glob("barrier__*.json")):
        ...
        try:
            meta_json, meta = load_interpolation_metadata(interp_csv)
        except Exception as e:
            unreadable_barriers.append({
                "barrier_json": str(barrier_json),
    ...
    out_df = pd.DataFrame(barrier_rows, columns=SUMMARY_COLUMNS)
