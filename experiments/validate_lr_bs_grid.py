...
def try_float(x: object) -> float | None:
    try:
        return float(x)
    except Exception:
        return None
...
def validate_one_run(run_dir: Path) -> list[str]:
    ...
    missing_training_keys = expected_keys_present(training_cfg, required_training_keys)
    for k in missing_training_keys:
        errors.append(f"{run_dir.name}: run_config.training missing key: {k}")
    ...
    rc_lr = try_float(training_cfg.get("learning_rate"))
    rc_bs = try_int(training_cfg.get("batch_size"))
    epochs = try_int(training_cfg.get("epochs"))
    if rc_lr is None:
        errors.append(f"{run_dir.name}: invalid training.learning_rate: {training_cfg.get('learning_rate')!r}")
    if rc_bs is None:
        errors.append(f"{run_dir.name}: invalid training.batch_size: {training_cfg.get('batch_size')!r}")
    if epochs is None:
        errors.append(f"{run_dir.name}: invalid training.epochs: {training_cfg.get('epochs')!r}")
    if errors:
        return errors
