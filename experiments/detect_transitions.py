...
def main() -> None:
    ...
    missing_required_cols = [c for c in REQUIRED_BARRIER_COLUMNS if c not in df.columns]
    if missing_required_cols:
        raise ValueError(
            f"Barrier summary is missing required columns: {missing_required_cols}"
        )

    available_jump_cols = [c for c in JUMP_COLUMNS if c in df.columns]
    missing_jump_cols = [c for c in JUMP_COLUMNS if c not in df.columns]
    if not available_jump_cols:
        raise ValueError(
            "Barrier summary does not contain any supported jump columns. "
            f"Expected one of: {JUMP_COLUMNS}"
        )
    ...
    for col in available_jump_cols:
    ...
    if REQUIRE_ALL_JUMPS:
        df["jump_signal"] = df[available_jump_cols].all(axis=1)
    else:
        df["jump_signal"] = df[available_jump_cols].any(axis=1)
    ...
    def first_triggered_jump(row: pd.Series) -> str:
        active = [c for c in available_jump_cols if bool(row[c])]
    ...
    summary: dict[str, Any] = {
        ...
        "rule": {
            ...
            "jump_columns_requested": JUMP_COLUMNS,
            "jump_columns_used": available_jump_cols,
            "missing_jump_columns": missing_jump_cols,
