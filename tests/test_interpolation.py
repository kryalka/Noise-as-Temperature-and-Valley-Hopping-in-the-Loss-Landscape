...
def _write_observed_interp_cfg(path: Path, *, data_root: str, num_points: int, eval_batch_size: int) -> None:
    text = dedent(f"""\
    data_root: {data_root}
...
def test_run_interpolation_observed_path_records_resolved_trajectory(
    tmp_path: Path,
    ...
    device = torch.device("cpu")
    eval_bs = 16
    fixed_loaders = _make_dummy_loaders(n_train=32, n_val=32, batch_size=eval_bs, seed=42)
    ...
    cfg_path = tmp_path / "observed_interpolation.yaml"
    _write_observed_interp_cfg(
        cfg_path,
        data_root="IGNORED",
        num_points=5,
    ...
    out_dir = tmp_path / "out"
    out_csv = run_interpolation(str(ckpt_a_path), str(ckpt_b_path), str(cfg_path), str(out_dir))

    assert out_csv.exists()
    assert "__observed__sel_all__" in out_csv.stem

    meta = json.loads(out_csv.with_suffix(".meta.json").read_text(encoding="utf-8"))
    observed = meta["path"]["observed"]

    assert meta["path"]["type"] == "observed"
    assert observed["selection"] == "all"
    assert observed["resolved_epochs"] == [1, 2, 3]
    assert len(observed["resolved_checkpoints"]) == 3
    assert observed["num_checkpoints"] == 3
    assert observed["num_segments"] == 2
    assert observed["parameterization"] == "arc_length_fraction"
    assert len(observed["segment_lengths"]) == 2
    assert len(observed["segment_endpoints_t"]) == 3
