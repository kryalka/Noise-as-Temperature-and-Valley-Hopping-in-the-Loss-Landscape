...
_RUN_NAME = "cifar10_resnet18_seed1__optsgd_lr0.1_bs128_wd0.0005_mom0.9_schnone__dummy"
...
def _write_interp_meta(
    ...
    *,
    ...
    ckpt_dir = root / "outputs" / "runs_lr_bs_grid" / _RUN_NAME / "checkpoints"
    ckpt_dir.mkdir(parents=True, exist_ok=True)

    ckpt_a = ckpt_dir / f"epoch_{epoch_a:03d}.pt"
    ckpt_b = ckpt_dir / f"epoch_{epoch_b:03d}.pt"
    ckpt_a.touch()
    ckpt_b.touch()
    ...
    interp_csv.with_suffix(".meta.json").write_text(
...
def test_compute_barrier_outputs_json_and_csv(tmp_path: Path) -> None:
    ...
    _write_interp_meta(interp_csv, root=tmp_path)
